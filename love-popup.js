if(typeof document!=='undefined'&&!window.__lovePopupV4){
  ...
  style.textContent=`
    .love-pop{position:fixed;left:50%;top:50%;z-index:10020;width:min(355px,calc(100vw - 28px));transform:translate(-50%,calc(-50% + 18px)) scale(.98);opacity:0;pointer-events:none;transition:opacity .22s ease,transform .28s cubic-bezier(.22,1,.36,1)}
    .love-pop.show{opacity:1;transform:translate(-50%,-50%) scale(1);pointer-events:auto}
    .love-pop-card{position:relative;display:grid;grid-template-columns:44px 1fr;gap:11px;align-items:center;background:rgba(255,255,255,.97);border:1px solid #f0dfe2;border-radius:21px;padding:12px 42px 12px 12px;box-shadow:0 18px 52px rgba(75,42,52,.20);backdrop-filter:blur(14px)}
  ...
  function readHash(){
    if(!location.hash.startsWith('#love='))return false;
  ...
  function replayPending(forceUpdate=false){
    readHash();
    requestPendingLove(forceUpdate);
  }
  if('serviceWorker' in navigator){
    ...
    navigator.serviceWorker.addEventListener('controllerchange',()=>setTimeout(()=>replayPending(),100));
    document.addEventListener('visibilitychange',()=>{if(!document.hidden)replayPending()});
    window.addEventListener('pageshow',()=>replayPending());
    window.addEventListener('focus',()=>replayPending());
    [80,500,1400,3200,6500].forEach((ms,i)=>setTimeout(()=>replayPending(i===0),ms));
  }
  ...
  import('./reminders.js?v=2').catch(()=>{});
  import('./games.js?v=1').catch(()=>{});
}
