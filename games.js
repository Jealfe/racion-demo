if(typeof document!=='undefined'&&!window.__familyGamesV1){
  ...
  .game-answer{width:100%;min-height:108px;border:1px solid #e8dfda;background:#fbf9f7;border-radius:16px;padding:13px;font:inherit;font-size:15px;resize:vertical;outline:none}.game-answer:focus{border-color:#bda7c3;box-shadow:0 0 0 3px rgba(164,129,174,.1)}.game-prompt-hint{margin:-7px 0 12px!important;color:#a09299!important;font-size:9px!important}
  ...
  .game-edge-wrap{margin:10px 0 0;border:1px dashed #cfc3d3;border-bottom:0;border-radius:14px 14px 0 0;overflow:hidden;background:#fff}.game-edge-label{font-size:8px;text-transform:uppercase;letter-spacing:.07em;color:#9c899f;padding:6px 9px;background:#f8f4fa}.game-edge-preview{display:block;width:100%;height:auto;aspect-ratio:9/1;object-fit:fill;background:#fff}
  .game-canvas-wrap{position:relative;border:1px solid #ded5df;border-radius:16px;overflow:hidden;background:#fff;touch-action:none}.game-edge-wrap+.game-canvas-wrap{border-radius:0 0 16px 16px}.game-canvas{display:block;width:100%;height:auto;aspect-ratio:4/5;background:#fff;touch-action:none}.game-share-guide{position:absolute;left:0;right:0;bottom:8.8889%;height:0;border-top:2px dashed rgba(128,86,141,.72);pointer-events:none;z-index:3}.game-share-guide span{position:absolute;right:6px;top:4px;padding:2px 5px;border-radius:7px;background:rgba(255,255,255,.9);color:#84678d;font-size:7px;font-weight:900;letter-spacing:.02em;white-space:nowrap;box-shadow:0 1px 4px rgba(61,42,68,.08)}.game-tools{display:flex;align-items:center;gap:7px;margin-top:9px;flex-wrap:wrap}.game-color{width:32px;height:32px;border-radius:50%;border:2px solid #fff;box-shadow:0 0 0 1px #d9d0cb;cursor:pointer;padding:0}.game-color.active{box-shadow:0 0 0 2px #7d6a83}.game-size{border:0;background:#f2eeea;border-radius:11px;padding:9px 11px;font-size:10px;font-weight:800;cursor:pointer}.game-size.active{background:#29272a;color:#fff}.game-tool-spacer{flex:1}
  #games.active .game-draw-turn{position:fixed;inset:0;z-index:4000;border:0;border-radius:0;box-shadow:none;overflow-y:auto;padding:calc(10px + env(safe-area-inset-top)) 12px calc(14px + env(safe-area-inset-bottom));background:#f8f6f4}.game-draw-inner{width:min(100%,680px);margin:0 auto}.game-draw-header{display:grid;grid-template-columns:42px 1fr;gap:10px;align-items:start;margin-bottom:4px}.game-draw-back{width:40px;height:40px;border:0;border-radius:13px;background:#fff;font-size:22px;font-weight:800;box-shadow:0 3px 12px rgba(50,40,35,.08);cursor:pointer}.game-draw-part{font-size:23px!important;line-height:1.05;margin:2px 0 5px!important}.game-draw-hint{font-size:10px!important;color:#8d8380!important;line-height:1.4!important}.game-draw-stage{width:min(100%,560px);margin:8px auto 0}.game-draw-bottom{width:min(100%,560px);margin:0 auto}.game-draw-turn .game-progress{margin:7px 0 8px}.game-draw-turn .game-primary{position:sticky;bottom:calc(8px + env(safe-area-inset-bottom));z-index:5;box-shadow:0 6px 22px rgba(30,25,28,.16)}
  @media(max-width:520px){.game-draw-turn .game-canvas{width:100%}.game-draw-part{font-size:21px!important}.game-draw-turn .game-tools{position:sticky;top:0;z-index:4;background:#f8f6f4;padding:8px 0 5px}}
  ...
  .game-story-line{margin-top:14px;border-radius:17px;background:#faf7f4;padding:15px;text-align:left}.game-story-line small{display:block;font-size:8px;color:#987f92;text-transform:uppercase;letter-spacing:.06em;margin-bottom:7px;font-weight:900}.game-story-line p{font-size:14px!important;line-height:1.55!important;color:#342f31!important;margin:0!important;font-weight:700}
  ...
  function myTurnHTML(active){
    if(active.mode==='words')return `<div class="game-card"><div class="game-step-label">${esc(active.scenario_title||'Словесная чепуха')} · Твой ход · ${Number(active.current_step)+1} из ${active.total_steps}</div>${progressHTML(active)}<p class="game-prompt">${esc(active.prompt)}</p>${active.hint?`<p class="game-prompt-hint">${esc(active.hint)}</p>`:''}<textarea class="game-answer" id="gameAnswer" maxlength="800" placeholder="Напиши свой вариант…"></textarea><button class="game-primary" id="gameSubmitWord">Отправить ответ</button><p class="game-sync-note">После отправки ответ спрячется до конца партии.</p><div class="game-actions">${stopButtonHTML()}</div></div>`;
    return `<div class="game-card game-draw-turn"><div class="game-draw-inner"><div class="game-draw-header"><button class="game-draw-back" id="gameDrawBack" type="button" aria-label="На главную">←</button><div><div class="game-step-label">Твоя часть · ${Number(active.current_step)+1} из ${active.total_steps}</div><h3 class="game-draw-part">${esc(active.prompt)}</h3><p class="game-draw-hint">${esc(active.hint||'Продолжи рисунок от видимого края.')}</p></div></div>${progressHTML(active)}<div class="game-draw-stage">${active.preview_data?`<div class="game-edge-wrap"><div class="game-edge-label">виден только край предыдущей части</div><img class="game-edge-preview" src="${esc(active.preview_data)}" alt="Край предыдущего рисунка"></div>`:''}<div class="game-canvas-wrap"><canvas class="game-canvas" id="gameCanvas" width="720" height="900"></canvas><div class="game-share-guide" aria-hidden="true"><span>ниже увидит партнёр</span></div></div></div><div class="game-draw-bottom"><div class="game-tools"><button class="game-color active" data-game-color="#252329" style="background:#252329" aria-label="Чёрный"></button><button class="game-color" data-game-color="#d76773" style="background:#d76773" aria-label="Красный"></button><button class="game-color" data-game-color="#5678aa" style="background:#5678aa" aria-label="Синий"></button><button class="game-color" data-game-color="#65916a" style="background:#65916a" aria-label="Зелёный"></button><span class="game-tool-spacer"></span><button class="game-size active" data-game-size="7">тонко</button><button class="game-size" data-game-size="14">толще</button></div><div class="game-actions"><button class="game-soft" id="gameUndo">↶ Отменить</button><button class="game-soft game-dangerless" id="gameClear">Очистить</button></div><button class="game-primary" id="gameSubmitDrawing" disabled>Отправить эту часть</button><p class="game-sync-note">Партнёр увидит только нижнюю полоску под пунктиром.</p><div class="game-actions">${stopButtonHTML()}</div></div></div></div>`;
  }
