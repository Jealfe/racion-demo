...
.list-item>.row{display:flex!important;align-items:center!important;flex-wrap:nowrap!important;gap:7px!important;justify-content:flex-end!important;margin-left:auto!important;min-width:80px!important}.list-item>.row .icon-btn{position:relative!important;display:grid!important;place-items:center!important;flex:0 0 36px!important;width:36px!important;height:36px!important;min-width:36px!important;min-height:36px!important;padding:0!important;line-height:1!important;transform:none}.list-item>.row .icon-btn:active{transform:scale(.94)!important}#ideaList .idea-del,#wishList .wish-del{font-size:19px!important;line-height:1!important;padding:0 0 2px!important}#ideaList .idea-done,#wishList .wish-done{font-size:16px!important}
...
function deleteLabel(el){if(el.closest('.idea-card'))return 'Удалить эту идею дизайна?';if(el.matches('.wish-del'))return 'Удалить эту хотелку?';if(el.closest('#ideaList'))return 'Удалить эту идею?';if(el.matches('.like-del'))return 'Удалить эту запись из «Нам нравится»?';if(el.matches('.moment-del'))return 'Удалить этот момент?';if(el.matches('.del-movie'))return 'Удалить этот фильм из списка?';return 'Удалить запись?'}
function askDelete(el){pendingDelete=el;confirmBox.querySelector('h3').textContent=deleteLabel(el);$('#deleteConfirmText').textContent=(localStorage.getItem('us_family_token')||'').trim()?'Она исчезнет из общей синхронизации у вас обоих.':'Она исчезнет с этого устройства.';confirmBox.classList.add('show')}
function closeDelete(){pendingDelete=null;confirmBox.classList.remove('show')}
