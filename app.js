...
const sectionNames={thanks:'Спасибо',wishlist:'Хотелки',ideas:'Идеи',likes:'Нам нравится',moments:'Моменты',movies:'Фильмы'};
const storageSections={'us_thanks':'thanks','us_wishes':'wishlist','us_ideas':'ideas','us_likes':'likes','us_moments':'moments','us_movies':'movies'};
function currentAuthor(){return normalizeAuthor(profile.name||'')}
function authorLabel(x){return normalizeAuthor(x?.author||'')}
...
$$('[data-home]').forEach(b=>b.addEventListener('click',()=>openScreen('home')));
...
function renderDesignLikes(){$$('.design').forEach(d=>{const b=d.querySelector('.heart'),liked=likedDesigns.has(d.dataset.id);b.textContent=liked?'♥':'♡';b.classList.toggle('liked',liked)})}
$$('.design .heart').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();const id=b.closest('.design').dataset.id;likedDesigns.has(id)?likedDesigns.delete(id):likedDesigns.add(id);store.set('designs',[...likedDesigns]);renderDesignLikes();toast(likedDesigns.has(id)?'Сохранил в понравившееся ❤️':'Убрал из понравившегося')}));
$$('.design').forEach(d=>d.addEventListener('click',()=>{const img=d.querySelector('img');$('#viewerImg').src=img.src;$('#viewerImg').alt=img.alt||'Идея интерьера';$('#viewerTitle').textContent=d.querySelector('.design-info b')?.textContent||'Идея интерьера';$('#designViewer').classList.add('show')}));
$('#closeViewer').addEventListener('click',()=>$('#designViewer').classList.remove('show'));$('#designViewer').addEventListener('click',e=>{if(e.target.id==='designViewer')$('#designViewer').classList.remove('show')});renderDesignLikes();
...
function updateThanksButtons(){const enabled=Boolean(currentThanks)&&!thanksBusy;$('#shareThanks').disabled=!enabled;$('#saveThanks').disabled=!enabled;$('#shareThanks').textContent=thanksBusy?'Открываю…':'Отправить 💌'}
function resetThanks(){currentThanks='';$('#thanksCustom').value='';$$('[data-thanks]').forEach(b=>b.classList.remove('active'));$('#thanksPreview').textContent=DEFAULT_THANKS_HINT;updateThanksButtons()}
function setThanks(reason){currentThanks=makeThanksText(reason,'');$('#thanksPreview').textContent=currentThanks;$('#thanksCustom').value='';$$('[data-thanks]').forEach(b=>b.classList.toggle('active',b.dataset.thanks===reason));updateThanksButtons()}
...
window.addEventListener('storage',e=>{
  ...
  let oldItems=[],newItems=[];try{oldItems=JSON.parse(e.oldValue||'[]')||[];newItems=JSON.parse(e.newValue||'[]')||[]}catch{return}
  const oldIds=new Set(oldItems.map(x=>String(x.refId||x.id||x.createdAt||'')));
  const added=newItems.filter(x=>!oldIds.has(String(x.refId||x.id||x.createdAt||''))&&authorLabel(x)&&authorLabel(x)!==currentAuthor());
