...
const legacyWordScenario = {
  ...
  prompts:[
    ...
    {q:'Где всё происходит?',hint:'Любое место'},
    {q:'Что происходит?',hint:'Любое действие или событие'},
    {q:'Что вдруг кто-то сказал?',hint:'Одна короткая фраза'},
    {q:'Что ответили?',hint:'Ещё одна независимая фраза'},
    {q:'Чем всё закончилось?',hint:'Например: все уехали есть пельмени'},
  ],
  build:(a:string[])=>`${a[0]}. ${a[1]}. ${a[2]}. И тут: «${a[3]}». В ответ: «${a[4]}». В итоге: ${a[5]}.`,
}
const wordScenarios = [
  {
    key:'classic', title:'Классическая чепуха',
    prompts:[
      {q:'Кто?',hint:'Любой персонаж'},
      {q:'Где всё происходило?',hint:'Например: в супермаркете ночью'},
      {q:'Что происходит?',hint:'Любое действие или событие'},
      {q:'Что вдруг кто-то сказал?',hint:'Одна короткая фраза'},
      {q:'Что ответили?',hint:'Одна короткая фраза'},
  ...
  },
  {
    ...
    prompts:[
      {q:'Кто?',hint:'Любой персонаж'},
      {q:'Что было с собой?',hint:'Любая вещь'},
      {q:'Где всё происходит?',hint:'Любое место'},
      {q:'Кто ещё появился?',hint:'Любой персонаж'},
      {q:'Что произошло дальше?',hint:'Не связывай с предыдущими ответами'},
      {q:'Чем закончилась эта странная история?',hint:'Чем нелепее — тем лучше'},
    ],
    build:(a:string[])=>`${a[0]}. С собой: ${a[1]}. Место: ${a[2]}. Там появился: ${a[3]}. Потом: ${a[4]}. В итоге: ${a[5]}.`,
  },
  {
    key:'dialogue', title:'Странный диалог',
    prompts:[
      {q:'Кто встретился?',hint:'Например: директор и говорящий кот'},
      {q:'Где?',hint:'Любое место'},
      {q:'Что сказал первый?',hint:'Одна короткая реплика'},
      {q:'Что ответил второй?',hint:'Одна короткая реплика'},
      {q:'Что произошло после этого?',hint:'Любое событие'},
      {q:'Чем закончилась встреча?',hint:'Короткий финал'},
    ],
    build:(a:string[])=>`${a[0]}. Место: ${a[1]}. Первый: «${a[2]}». Второй: «${a[3]}». После этого: ${a[4]}. В итоге: ${a[5]}.`,
  },
  {
    key:'bad_plan', title:'Плохой план',
    prompts:[
      {q:'Кто?',hint:'Любой персонаж'},
      {q:'Какой был план?',hint:'Любая задумка'},
      {q:'Где всё происходило?',hint:'Любое место'},
      {q:'Что пошло не по плану?',hint:'Например: приехал цирк'},
      {q:'Что было дальше?',hint:'Независимое действие'},
      {q:'Чем всё закончилось?',hint:'Короткий финал'},
    ],
    build:(a:string[])=>`${a[0]}. План: ${a[1]}. Место: ${a[2]}. Но: ${a[3]}. Потом: ${a[4]}. В итоге: ${a[5]}.`,
  },
  {
    key:'weird_day', title:'Один безумный день',
    prompts:[
      {q:'Кто?',hint:'Любой персонаж'},
      {q:'Где проснулся?',hint:'Любое место'},
      {q:'Что первым делом сделал?',hint:'Например: позвонил маме'},
      {q:'Что увидел потом?',hint:'Что угодно'},
      {q:'Что решил сделать?',hint:'Любое решение'},
      {q:'Как закончился его день?',hint:'Короткий финал'},
    ],
    build:(a:string[])=>`${a[0]}. Проснулся: ${a[1]}. Первым делом: ${a[2]}. Потом увидел: ${a[3]}. Решил: ${a[4]}. Финал дня: ${a[5]}.`,
  },
  {
    key:'date', title:'Свидание',
    prompts:[
      {q:'Кто пришёл на свидание?',hint:'Можно придумать сразу двоих'},
      {q:'Где было свидание?',hint:'Любое место'},
      {q:'Что было с собой?',hint:'Любая вещь'},
      {q:'Что неловкое произошло?',hint:'Например: перепутали столик'},
      {q:'Что кто-то сказал?',hint:'Одна короткая реплика'},
      {q:'Чем закончилось свидание?',hint:'Романтично или максимально нелепо'},
    ],
    build:(a:string[])=>`${a[0]}. Место встречи: ${a[1]}. С собой: ${a[2]}. И тут: ${a[3]}. Прозвучало: «${a[4]}». В итоге: ${a[5]}.`,
  },
  {
    key:'superpower', title:'Суперспособность',
    prompts:[
      {q:'Кто получил суперспособность?',hint:'Любой персонаж'},
      {q:'Какую способность?',hint:'Например: разговаривать с холодильниками'},
      {q:'Где всё происходило?',hint:'Любое место'},
      {q:'Что получилось не так?',hint:'Например: исчезли все стулья'},
      {q:'Кто что сказал или сделал?',hint:'Любая реакция'},
      {q:'Чем всё закончилось?',hint:'Короткий финал'},
    ],
    build:(a:string[])=>`${a[0]}. Способность: ${a[1]}. Место: ${a[2]}. Но: ${a[3]}. Реакция: ${a[4]}. В итоге: ${a[5]}.`,
  },
  {
    ...
    prompts:[
      ...
      {q:'Кого встретил?',hint:'Любой персонаж'},
      {q:'Что произошло дальше?',hint:'Любое событие'},
      {q:'Как всё закончилось?',hint:'Любой финал'},
    ],
    build:(a:string[])=>`${a[0]}. Направление: ${a[1]}. С собой почему-то: ${a[2]}. Встреча: ${a[3]}. Потом: ${a[4]}. Финал: ${a[5]}.`,
  },
]
